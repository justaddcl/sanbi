...
const GLOBAL_SEARCH_DIALOG_TAG_RESERVATION = 3;
...
export const getVisibleGlobalSearchResults = <SongResult, TagResult>({
  ...
  const reservedTagCount =
    tagResults.length > 0
      ? Math.min(GLOBAL_SEARCH_DIALOG_TAG_RESERVATION, tagResults.length)
      : 0;
  const visibleSongResults = songResults.slice(
    0,
    GLOBAL_SEARCH_DIALOG_RESULT_LIMIT - reservedTagCount,
  );
